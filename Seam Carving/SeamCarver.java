...
public class SeamCarver {
	...
	private Picture removeSeam(Picture picture, int[] seam) {
		Picture newPicture = new PictureAs2DArray(picture.getWidth() - 1, picture.getHeight());
		for (int y = 0; y < picture.getHeight(); y++) {
			int i = 0;
			for (int x = 0; x < picture.getWidth(); x++) {
				if (x != seam[y]) {
	...
	public Picture addHorizontalSeam(Picture picture, int[] seam, Color seamColor) {
		Picture newPicture = new PictureAs2DArray(picture.getWidth(), picture.getHeight() + 1);
